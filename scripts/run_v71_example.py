"""Script that can be used to run Aerugo example on remote SAMV71 board connected via
OpenOCD probe.

Usage:
Run this script from Aerugo's root directory. Pass the directory name of the example as
an argument (for example, `samv71-basic-execution`). This should be a preferred way of
running the examples, or should be treated as template for manual execution of any
...
def get_example_path(example_name: str) -> Path:
    """Returns path to the project directory of provided example."""
...
def load_and_start_program(gdb: GDBClient, program_path: Path) -> None:
    ...
    if not program_path.exists():
        ...
        sys.exit(ExitReason.EXAMPLE_NOT_BUILT)
...
def main():
    ...
    program_path = build_cargo_app(example_path, BOARD_TARGET_TRIPLE, release_build=True)
